...
const areValuesContained = (input: string) => {
  ...
  for (let i = 0; i < pairs.length; i++) {
    ...
    if (
      ...
      parseInt(assignments[0][0]) <= parseInt(assignments[1][1]) ||
      parseInt(assignments[1][0]) >= parseInt(assignments[0][0]) &&
      parseInt(assignments[1][0]) <= parseInt(assignments[0][1])
    ) {
      pairCount++;
      console.log("countup");
    }
